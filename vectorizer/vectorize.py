...
# STEP 4 - Embed the products and create the documents
...
def build_metadata(p: dict) -> dict:
    ...
    # Effects - keep as array, normalize to lowercase
    effects = p.get("effects")
    if effects:
        if isinstance(effects, list):
            metadata["effects"] = [str(e).lower() for e in effects if e]  # Normalize to lowercase
        else:
            metadata["effects"] = [str(effects).lower()]  # Convert single value to array and lowercase
    else:
        metadata["effects"] = []
